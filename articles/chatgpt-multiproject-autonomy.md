...
1か月で813件のPull Requestを作り、686件のPull Requestをmergeした。

この数字だけを見ると、「AIでコードを書く速度が上がった話」に見える。

でも、実際に一番重くなったのはコードを書く工程ではなかった。

GitHubには公開リポジトリが146個ある。そのうち、forkを除く123個が自作側の個人開発だ。

金融、VRChat、ボードゲーム、動画、家計、旅行、論文、3D制作、データ収集、AI agent。

プロジェクトが増えるほど、毎回こう考える時間が増えた。
...
そこで2026年夏、実装だけではなく、あるrepoのIssueを読み、次に進める作業を選び、実装し、CIを確認し、PRをmergeし、productionを確認し、終わったら別のrepoを見るところまでChatGPTを介して扱うようになった。

**なぜ、もともと何の関係もなかった大量の個人プロジェクトを、一つの制御ループで扱えるようになったのか。**

この記事で追うのは、その理由だ。

なお、813 PRという件数は品質の証明ではない。大量にPRを作ったこと自体を成果には数えない。

むしろこの記事の問いは逆である。

**AIがコードを書くほど速くなるのに、なぜ人間の調整仕事まで増えずに済んだのか。**
...
ところがFRED公式の当時の表示を確認すると、その時点で利用可能だった最新観測は `2026-07-23 = 4.71` で、7月24日の4.69は後の更新で初めて現れていた。
...
### ここでいう「ChatGPTに任せた」の境界

ここで、誤解しやすい境界を一つ置いておく。

何も設定していない標準チャットが、123 repoを勝手に巡回して優先順位やDone条件を発明する、という意味ではない。

この運用では、GitHub側へIssue、PR、Actions、main、production、証拠、権限境界を出し、接続したChatGPT / Codex側がその時点の状態を読んで次の1件を扱う。

つまり、AIに渡しているのは「全部覚えて、いい感じにやって」という曖昧な仕事ではない。

**状態と契約が外部化された仕事を、現在状態から再開する役割**である。

この違いは大きい。AIモデルが変わっても、会話が切れても、repo側の契約が残っていれば再開地点を作れる。
...
この話は123 repoある人だけのものではない。

むしろ5個、10個と増えて「前回どこまでやったか」を頭の中で持つのがつらくなった段階から効く。

最低限、各repoに次の4つがあればよい。

```text
1. Issue
   何を変えるか + 何をもってDoneとするか

2. 機械検証
   test / lint / build / CIなど、機械が証明できる完了条件

3. 利用者側の確認
   production / Pages / API / artifactなど、実際に使われる状態の確認

4. 人間へ返す境界
   公開、売買、削除、creative choiceなど、機械に決めさせない判断
```

この4つが揃えば、次の会話でAIが前の会話を覚えているかより、**repoを読めば現在地が分かるか**の方が重要になる。

逆に、123 repoあっても、Done条件と証拠が人間の頭の中にしかなければ、AIは毎回説明を聞き直す。

個人開発を増やす前に作るべき共通基盤は、巨大なagent frameworkではなく、この小さな「再開可能性」かもしれない。
...
個人開発が増えすぎたとき、必要だったのは、全部を覚えてくれる巨大なAIではなかった。

**忘れても再開できるrepoだった。**

次に何をするかを毎回、自分の頭の中へ置いておかなくていい。

一方で、何を作るか、何を公開するか、どこで止めるかは人間に残る。
