---
title: "『モデル差し替えは後日』と思っていたら、もう公式対応していた：VRChatペットを犬化する前に設計を引き直した"
emoji: "🐶"
type: "tech"
...
ところが、2026年8月12日に販売元の現行BOOTHページを確認すると、この前提はすでに崩れていました。

ムチォの現行版は **2026-08-05 v1.3.6**。商品ページには同梱物として **「オリジナルペットの作り方」** が明記され、同梱モデルの改変も許可されています。同期パラメータ使用量は **77bit**、導入はModular Avatar対応Prefabをアバター直下へ置く方式です。

- ムチォ商品ページ: https://booth.pm/ja/items/8657397

つまり、問題は「どうやって強引にモデルを差し替えるか」ではありませんでした。

**既存の犬Prefabが持っている耳・尻尾・アニメーション・接触挙動を壊さず、ムチォの追従・発話・インタラクションだけをどう接続するか。**

設計の中心を、モデル置換から **Core / Body分離** へ変える必要がありました。

この記事では、その判断がどこから出てきたのか、そして実装前に `happy_shiroi-nu_PC` の何を観測すべきかを整理します。

## 1. まず一次情報を更新したら、問題設定そのものが変わった

ムチォの現行BOOTHページで確認できる重要な事実は4つあります。
