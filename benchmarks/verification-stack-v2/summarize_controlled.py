...
def main() -> None:
    ...
    for record in source["results"]:
        ...
        if record["baseline_blocking_false_positive"]:
            baseline_failures.append({"candidate": candidate, "mutant_id": record["mutant_id"], "kind": "source_baseline"})
        if "format_quality" in record:
            q = record["format_quality"]
            format_quality[f"{record['mutant_id']}::{candidate}"] = {
                "matches_candidate_normalized_baseline": q["matches_candidate_normalized_baseline"],
                "idempotent": q["idempotent"],
                "smoke_pass": q["smoke_pass"],
                "patch_sha256": q["patch_sha256"],
            }
            if not (q["matches_candidate_normalized_baseline"] and q["idempotent"] and q["smoke_pass"]):
                baseline_failures.append({"candidate": candidate, "mutant_id": record["mutant_id"], "kind": "formatter_calibration"})

    hook_parity = hooks["parity"]
    if not hook_parity.get("both_idempotent", False):
        baseline_failures.append({"candidate": "pre-commit/prek", "mutant_id": "HOOK-PATCH-PARITY-001", "kind": "hook_baseline"})

    discovery = {item["candidate"]: bool(item["correct"]) for item in workspace.get("discovery", [])}
    for candidate in ("nx", "turbo"):
        if discovery.get(candidate) is not True:
            baseline_failures.append({"candidate": candidate, "mutant_id": "WORKSPACE-DISCOVERY", "kind": "workspace_discovery"})
    ...
    baseline_calibration = {
        ...
        "rule": "No controlled result is recommendation-eligible unless every declared clean baseline and both workspace discovery gates pass.",
    }
    ...
    (RESULTS / "baseline-calibration.json").write_text(json.dumps(baseline_calibration, indent=2, sort_keys=True) + "\n", encoding="utf-8")
