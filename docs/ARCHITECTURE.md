...
内部スコアはLAPRAS AI Reviewの実測値ではなく、公開5軸を参考にしたproxyです。月末は全候補を同じ3回査読条件で再評価し、品質ゲートを通る集合だけから1本を選びます。合格候補が0件なら公開も0件です。
...
GitHub Models inference APIは2026-07-30に終了しているため、canonical pipelineでは使用禁止です。
